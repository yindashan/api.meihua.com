...
class OSS {
    ...
    private function _get_upload_name($name) {
        mt_srand((double)microtime()*10000);//optional for php 4.2.0 and up.
        $uuid = strtoupper(md5(uniqid(rand(), true)));
        return $uuid.'.'.pathinfo($name, PATHINFO_EXTENSION);
    }
    ...
    function upload_tweet_pic ($file, $name) {
        ...
        $ret = $this->upload_img(self::IMG_BUCKET, 
                                 $file, 
                                 'tweet/'.$this->_get_upload_name($name));  
        if (false === $ret) {
            return $ret; 
        }
        $result = array();
    ...
    function upload_user_pic ($file, $name) {
        ...
        $ret = $this->upload_img(self::IMG_BUCKET, 
                                 $file, 
                                 'user/'.$this->_get_upload_name($name));  
        if (false === $ret) {
            return $ret;  
        }
        $result = array();
    ...
    function upload_mis_pic ($file, $name) {
        $ret = $this->upload_img(self::IMG_BUCKET, 
                                 $file, 
                                 'mis/'.$this->_get_upload_name($name));  
        if (false === $ret) {
            return $ret; 
        }
        $result = array();
    ...
    function upload_crash($file, $name) {
        return $this->upload(self::INTERNAL_BUCKET, 
                             $file, 
                             'crash/'.$this->_get_upload_name($name));  
    }

    function upload_log($file, $name) {
        return $this->upload(self::INTERNAL_BUCKET, 
                             $file, 
                             'log/'.$this->_get_upload_name($name));  
    }
