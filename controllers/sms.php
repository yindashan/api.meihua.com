...
class Sms extends MY_Controller {
    ...
    private function _check_send_sms($mobile) {
        ...
        log_message('error', 'max_sms_count:'.$max_sms_count);
        log_message('error', 'user_sms_count:'.$user_sms_count);
    ...
    public function send() {
        ...
        //符合条件，发送短信
        if(false !== $this->_check_send_sms($mobile)) {
            ...
            if(!isset($template_arr[$type]) 
                ...
                log_message('error', __METHOD__ . ' sms tpl error, errno[' . $this->response['errno']. ']');
                goto end;
            }
            ...
            log_message('error', 'sms_content:'.$sms_content);

            //发送短信todo

            //写MySQL
            $sid = $this->_set_sms($uid, $mobile, $sms_code, $operate);
            if(false === $sid) {
                $this->response['errno'] = MYSQL_ERR_INSERT;
                log_message('error', __METHOD__ . ' set sms error, mobile['.$mobile.'] errno[' . $this->response['errno'] .']');
                goto end;
            }
        }
    ...
    public function verify() {
        ...
        $sms_info = $this->Sms_model->get_info_by_verifycode($mobile, $verifycode);
        log_message('error', var_export($sms_info, true));
        if(false === $sms_info) {
            $this->response['errno'] = MYSQL_ERR_SELECT;
