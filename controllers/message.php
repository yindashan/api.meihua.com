...
class Message extends MY_Controller {
    ...
    function usermsg() {
        ...
        foreach ($ret as $talk_info) {
            $sname = '';
            $avatar = '';
            //$ret = $this->cache_model->get_user_info($talk_info['other_uid'], $fields);
            $ret = $this->cache_model->get_user_detail_info($talk_info['other_uid'], $fields);
            if(!$ret) {
                $response['errno'] = REDIS_ERR_OP;
        ...
        //$this->msclient->clear_red_by_uid($uid, 8, 0);
    }
    ...
    function newmsg() {
        ...
        // 4. 推送到消息中心
        $this->msclient->send_system_msg($from_uid, ACTION_TYPE_MSG, intval($to_uid), $mid);
    ...
    private function _fill_user_info($msgs, $uid, $other_uid) {
        $user_info = $this->cache_model->get_user_info($uid, array('sname', 'avatar'));
        if (!$user_info) {
            $user_info = array('sname' => '', 'avatar' => ''); 
        }
        $other_user_info = $this->cache_model->get_user_info($other_uid, array('sname', 'avatar'));
        if (!$other_user_info) {
    ...
    private function talk_load ($uid, $other_uid, $rn) {
        ...
        #log_message('error', 'message_talk_load_mids:'.json_encode($mids));
    ...
    function talk() {
        ...
        $this->msclient->clear_red_by_uid($uid, 8, 0, intval($other_uid));
    }
    ...
    function delmsg() {
        ...
        if($this->_uid !== $uid) {
            ...
            log_message('error', __METHOD__ . ' user illegal, uid['.$uid.'] touid['.$to_uid.'] errno[' . $response['errno'] .']');
            goto end;
        }
         */
        ...
        if(!$result) {
            ...
            $this->msclient->clear_red_by_uid($uid, 8, 0, intval($to_uid));
        }
        end:
        $this->renderJson($response['errno']);
    }
