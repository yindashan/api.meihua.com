...
class Relation extends MY_Controller {
    ...
    private function single_follow($follower_uid, $followee_uid) {
        ...
        if (!$res) {
            // insert relation
            log_message('debug', 'wal_ice: in insert.');
            ...
            // update relation
            log_message('debug', 'wal_ice: in update, cur_followed_status: '.strval($cur_followed_status));
        ...
        if ($this::MUTUAL_FOLLOW == $new_follow_type) {
            $res = $this->Relation_model->update($follower_uid, $uid, array(
                    'follow_type' => $this::MUTUAL_FOLLOW, 
                )); 
            if (false === $res) {
                return false; 
            }
    ...
    function followee_list() {
        ...
        foreach ($res as $row) {
            $id = $row['uid'];
            $user_info = array();
            $user_detail = $this->cache_model->get_user_detail_info($id, array('sname', 'avatar'));
    ...
    function follower_list() {
        ...
        foreach ($res as $row) {
            ...
            $user_ext = $this->cache_model->get_user_ext_info($id);
            if (false === $user_ext || NULL === $user_ext) {
                continue;
            }
            $user_info['uid'] = $id;
            $user_info['follow_type'] = $row['follow_type'] ? $this::MUTUAL_FOLLOW : $this::ONE_WAY_FOLLOW;
            $user_info['name'] =  $user_detail['sname'];
            $user_info['img'] =  $user_detail['avatar'];
            $user_info['followee_num'] = intval($user_ext_info['followee_num']);
            $user_info['follower_num'] = intval($user_ext_info['follower_num']);
            $user_info['tweet_num'] = intval($user_ext_info['tweet_num']);
