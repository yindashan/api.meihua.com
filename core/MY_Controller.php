...
#        $this->load->library('msclient');
    ...
    function get_tweet_detail($tid) {
        ...
        log_message('error', 'tweet_res:'.json_encode($result));
        ...
        if(!empty($result)) {
            ...
            if(isset($tweet['origin_tid']) && !empty($tweet['origin_tid'])) {
                $origin_tweet = $this->tweet_model->get_tweet($tweet['origin_tid']);
            ...
            log_message('error', 'test-----'.json_encode($img1));
             */

            $tweet['imgs'] = $img;
            $tweet['picnum'] = count($img);
            //$tweet['body'] = $tweet['content'];
            //$tweet['img'] = $this->get_img($tweet['img']);
            /*
            if(is_array($tweet['img'])) {
                $tweet['img'] = $tweet['img'][0];
            }
             */
    ...
    function get_user_by_uid($uid) {
        $this->load->model('cache_model');
        $fields = array('uname', 'sname', 'avatar', 'ukind', 'ukind_verify', 'company', 'intro');
        $user_data = $this->cache_model->get_user_detail_info($uid, $fields);
